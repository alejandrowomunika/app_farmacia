...
class _CategoryPageState extends State<CategoryPage> {
  ...
  Map<String, List<String>> _subcategoryDescendants = {}; // Mapa de subcategoría -> todos sus descendientes
    ...
    try {
      ...
      final List<Map<String, dynamic>> products = items.map((it) {
        ...
        return {
          ...
          'price': price,
          'image': imgUrl,
          'categoryId': categoryId, // ← IMPORTANTE: Guardar la categoría del producto
        };
      }).toList();
  ...
  void _applyFilters() {
    ...
    if (_selectedSubcategoryId != null) {
      final allowedCategories = _subcategoryDescendants[_selectedSubcategoryId] ?? [_selectedSubcategoryId!];
    ...
    if (_searchText.isNotEmpty) {
      result = result.where((p) {
        return p['name'].toString().toLowerCase().contains(_searchText.toLowerCase());
      }).toList();
    }

    // Aplicar límite
    final limited = result.length > _maxProducts ? result.sublist(0, _maxProducts) : result;
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          const SizedBox(width: 6),
          Flexible( // ← CAMBIA DE Text directo a Flexible > Text
            child: Text(
              label,
              style: AppText.small.copyWith(
...
// ═══════════════════════════════════════════════════════════════════════════════
// WIDGET: TARJETA DE PRODUCTO (sin cambios)
// ═══════════════════════════════════════════════════════════════════════════════
